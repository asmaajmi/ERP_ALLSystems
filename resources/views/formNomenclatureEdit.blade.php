...
<div class="tablesformnomenclature">
	<form action="{{route('Nomenclature.update',['nomenclature'=>$nomenclature->id])}}" method="post">
		@csrf
		<div class="ajout_nomen">
				...
				<tr>
					...
					<td>
						<div class="champserv2">
							<input type="text" name="nom_nomenclature" value="{{$nomenclature->designation}}" id="champ_Nomenclature">
						</div>
					</td>
			...
			<table class="tableprod_const">
				@foreach ($constituers as $const)
				@foreach ($produit_const as $produit)
				@if ($const->id_prodconstruisable == $produit->id && $const->id_nomenclature== $nomenclature->id)
				<tr >      
					<td>
					...
					<td>
						<div class="champserv2" >
							<select type="number" name="nom_produit_const[]" id="champ_nom_prod" class="nom_produit">
								...
								@foreach ($produit_const as $produit)
								@if ($const->id_prodconstruisable == $produit->id)
								<option value="{{$produit->id}}" selected>{{$produit->nom_produit_const}}</option>
								@else
					...
					<td>
						<div class="champserv2" >
							<input type="number" name="quantite_produit_const[]" id="champ_quantite" value="{{$const->quantite}}">
						</div>
					</td>
					<td>
					...
					<td>
						<div class="champserv2" >
							<input type="number" name="arrondi_produit_const[]" id="champ_arrondi" value="{{$const->arrondi}}">
						</div>
					</td>
					<td>
					...
					<td>
						<div class="champserv2" >
							<input type="text" name="unite_produit_const[]" id="champ_unite" value="{{$const->unite}}">
						</div>
					</td>
			...
			<table class="tableprod_achet">
				...
				@if ($const->id_prodachetable == $produit_ach->id && $const->id_nomenclature== $nomenclature->id)
				<tr >      
					...
					<td>
						<div class="champserv2" >
							<select type="number" name="nom_produit_achet[]" id="champ_nom_prod" class="nom_produit">
								...
								@foreach ($produit_achetable as $produit_ach)
								@if ($const->id_prodachetable == $produit_ach->id)
								<option value="{{$produit_ach->id}}" selected>{{$produit_ach->nom_produit}}</option>
								@else
					...
					<td>
						<div class="champserv2" >
							<input type="number" name="quantite_prod_achet[]" id="champ_quantite" value="{{$const->quantite}}">
						</div>
					</td>
					<td>
						<h3 class="formtxt" id="arrondi" >Arrondi</h3>
					...
					<td>
						<div class="champserv2" >
							<input type="number" name="arrondi_prod_achet[]" id="champ_arrondi" value="{{$const->arrondi}}">
						</div>
					</td>
					<td>
						<h3 class="formtxt" id="unite"  >Unité<span id="obligatoire">*</span></h3>
					...
					<td>
						<div class="champserv2" >
							<input type="text" name="unite_prod_achet[]" id="champ_unite" value="{{$const->unite}}"}>
						</div>
					</td>
...
$(document).ready(function(){
		...
		var tr ='<tr >'+     
					'<td>'+
						'<h3 class="formtxt" id="nom_prod" >Nom de produit</h3>'+
					'</td>'+
					'<td>'+
						'<div class="champserv2" >'+
							'<select type="number" name="nom_produit_achet[]" id="champ_nom_prod" class="nom_produit">'+
					...
					'<td>'+
						'<h3 class="formtxt" id="quantite" >Quantité</h3>'+
					'</td>'+
					'<td>'+
						'<div class="champserv2" >'+
					...
					'</td>'+
					'<td>'+
						'<h3 class="formtxt" id="Arrondi"  >Unité</h3>'+
					'</td>'+
					'<td>'+
						'<div class="champserv2" >'+
