...
class PonctualitePersonnelleAnnuelleController extends Controller
{
    public function CalculerPonctuaAnnuelle(Request $request){
        
        $annee=$request->input('annee_ponct');
        $id_employe=$request->input('id_employe2');
        ...
        $count = DB::table('ponctualite_personnelle_annuelles')->where('id_emp',$id_employe)
                                                         ->where('annee',$annee)
                                                         ->count(DB::raw('id'));
        
        if($count>0){
            PonctualitePersonnelleAnnuelle::where('id_emp',$id_employe)
                                    ->where('annee',$annee)
                                    ->delete();}
        // fin de verification
    ...
    //  fin de jour dimanche
        ...
        $notePonctAnnuelle = 1- $formuledenote;

        if($notePonctAnnuelle<=1 && $notePonctAnnuelle>=0.95)
        {
            $mention="Excellent";
        }
        else if($notePonctAnnuelle<0.95 && $notePonctAnnuelle>=0.90)
        {
            $mention="Bon";
        }
        else if($notePonctAnnuelle<0.90 && $notePonctAnnuelle>=0.85)
        {
            $mention="Moyen";
        }
        else if($notePonctAnnuelle<0.85 && $notePonctAnnuelle>=0.8)
        {
            $mention="Faible";
        }
        else
        ...
        $ponctualite_pers_annuelle=PonctualitePersonnelleAnnuelle::create([
            'annee'=>$annee,
            'valeur'=>$notePonctAnnuelle,
            'mention'=>$mention,
            'id_emp'=>$id_employe
        ]);
        if($ponctualite_pers_annuelle){
