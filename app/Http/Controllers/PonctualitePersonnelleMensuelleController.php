...
class PonctualitePersonnelleMensuelleController extends Controller
{
    public function CalculerPonctuaMensuelle(Request $request){
        ...
        if($countd != 0){
            ...
            if($datenb == 0){
                Alert::error('Erreur', "Cet employé n'a aucun pointage dans cet mois vous-pouvez choisir un autre mois");
                return back();}

            

        // verifier si la note est calcule déjà si c'est le cas en efface la ligne qui existe dans la table déjà
        $count = DB::table('ponctualite_pers_mensuelles')->where('id_emp',$id_employe)
                                                         ->where('annee',$annee)
                                                         ->where('mois',$mois)
                                                         ->count(DB::raw('id'));
        
        if($count>0){
            PonctualitePersMensuelle::where('id_emp',$id_employe)
                                    ->where('annee',$annee)
                                    ->where('mois',$mois)
                                    ->delete();}
        // fin de verification
    ...
    //  fin de jour dimanche
        ...
        $notePonctMensuelle = 1- $formuledenote;

        if($notePonctMensuelle<=1 && $notePonctMensuelle>=0.95)
        {
            $mention="Excellent";
        }
        else if($notePonctMensuelle<0.95 && $notePonctMensuelle>=0.90)
        {
            $mention="Bon";
        }
        else if($notePonctMensuelle<0.90 && $notePonctMensuelle>=0.85)
        {
            $mention="Moyen";
        }
        else if($notePonctMensuelle<0.85 && $notePonctMensuelle>=0.8)
        {
            $mention="Faible";
        }
        else
        ...
        $ponctualite_pers_mensuelle=PonctualitePersMensuelle::create([
            'annee'=>$annee,
            'mois'=>$mois,
            'valeur'=>$notePonctMensuelle,
            'mention'=>$mention,
            'id_emp'=>$id_employe
        ]);
        if($ponctualite_pers_mensuelle){
